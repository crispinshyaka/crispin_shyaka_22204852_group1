...
INSERT INTO `recipes` (`recipe_id`, `title`, `ingredient_lst`, `prepqraton_time`, `cooking_time`, `nutritional_info`, `user_rating`, `instructions`) VALUES
(1, 'breakfast', 'meat_onions_eggs', NULL, 1, NULL, 20, NULL),
(2, 'lunch', 'rice\nmeat\nbeans', NULL, NULL, NULL, NULL, 'boil rice \nmix meet with cerears'),
(3, 'supper', 'cassava_beans', NULL, NULL, NULL, NULL, 'mix and boil for 30mins'),
(1, 'breakfast', 'meat_onions_eggs', NULL, 1, NULL, 20, NULL);
...
,`paword` char(0)
,`preferrence` char(0)
,`cooking_level``cooking_level` char(0)
);
...
(2, 'junior', 'johnson', 'juniorjun@', 'goat', 'sou', 'worst'),
...
(7, 'james', 'rukundo', 'ruk@gmail.com', 'ahaaa', '', ''),
(8, 'crispin', 'shyaka', 'cris@gmail.com', 'ihene', NULL, NULL),
(1, 'karangwa', 'umutoni', 'ritaumutogmail.com', 'rita@123', 'snacks', 'bad'),
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `insertusers`  AS SELECT `users`.`user_id` AS `user_id`, `users`.`first_name` AS `first_name`, `users`.`last_name` AS `last_name`, `users`.`emal` AS `emal`, `users`.`paword` AS `paword`, `users`.`preferrences` AS `preferrences`, `users`.`cooking_level` AS `cooking_level` FROM `users``users`  ;
